...
function backgroundProcess(scmUrl) {
    const process = require('child_process');   // The power of Node.JS

    // showOS();
    var cmd = (is.windows()) ? `./src/scripts/import-repo.bat` : `./src/scripts/import-repo.sh`;      
        
    var child = process.spawn(cmd, [scmUrl]);
    ...
    child.on('close', function (code) {
        if (code == 0)
          ...
          setStatus('Import process exited with code ' + code);
    });
};